...
rna_ch <- readRDS("data_clean/ch/lab_ch.rds") %>% 
  mutate(
    cohort = as.factor("CH"),
    cd4_trans = sqrt(cd4),
...
rna_ch <- as.data.table(rna_ch)

rna_rsa <- readRDS("data_clean/rsa/lab_both_rsa.rds") %>% 
  mutate(
...
model_simple_ch <- lme4::lmer(rna_trans ~ poly(time_trans, 3) * presenting_tb + (1 |id), 
                              data = rna_ch)

model_simple_rsa <- lm(rna_trans ~ poly(time_trans, 3) * presenting_tb, 
                       data = rna_rsa)
...
pred_comp <- pred_both %>% 
  ...
  labs(x = "Days after ART start",
       y = expression("Viral Load (log10 copies/ml)"),
...
ggsave(pred_comp, filename = "results/rna_slope_complete.png",
...
fill_values_for_id <- function(patient_data, global_grid, max_value = 480, md) {
  ...
  results <- map_df(global_grid, function(grid_day) {
    ...
    if (nrow(interval_data) > 0) {
      ...
      tibble(time_trans = random_time_trans, rna_trans = NA_real_)
    }
  })
...
predictions.ch <- lapply(seq_along(imp.list_CH$imputations), function(i) {
  ...
  m <- lmer(rna_trans ~ poly(time_trans, 2) * presenting_tb + (1 | clus), 
...
saveRDS(pooled_predictions.ch, "results/pooled_predictions_rna_ch.rds")
...
md_intvstRSA <- intervisit_Md(rna_rsa) 
...
predictions.rsa <- lapply(1:K, function(i) {
  ...
  data <- mice::complete(rsa.imp, action = i) %>% 
    pivot_longer(cols = c(rna_trans_first, rna_trans_second, time_trans_first, time_trans_second),
                 names_to = c(".value", "measurement"),
                 names_pattern = "(.*)_(.*)")
  
  # Fit the model to the ith imputed dataset
  m <- lm(rna_trans ~ poly(time_trans, 2) * presenting_tb, 
          data = data)
...
saveRDS(pooled_predictions.rsa, "results/pooled_predictions.rna.rsa.rds")
...
#pooled_predictions.ch <- readRDS("results/pooled_predictions_rna_ch.rds")
#pooled_predictions.rsa <- readRDS("results/pooled_predictions.rna.rsa.rds")
...
plot
ggsave(plot, filename = "results/rna_slope.png",
