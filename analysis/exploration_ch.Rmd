...
incident <- art %>% 
  filter(case_incident_2m == "Incident TB")
```
...
cd4_base_tb <- baseline(presenting)
...
cd4_yearFIT <- boot_df.cd4 %>%
  ...
  geom_ribbon(aes(ymin = lower_bound.cd4, ymax = upper_bound.cd4), alpha = 0.2, fill = "red") +
  geom_line(aes(y = fitted.cd4_spline), color = "blue") +
  geom_ribbon(aes(ymin = lower_bound.cd4_spline, ymax = upper_bound.cd4_spline), alpha = 0.2, fill = "blue") +
  theme_bw() +
...
cd4_yearFITzoomed <- cd4_yearFIT +
  scale_x_continuous(limits = c(0,30), expand = c(0,0)) +
  scale_y_continuous(limits = c(15,20), expand = c(0,0)) +
  labs(title = "LMEM (red) + GAM with a spline (blue) rescaled")

print(cd4_yearFITzoomed)
...
rna_yearFIT <- boot_df.rna %>%
  ...
  geom_ribbon(aes(ymin = lower_bound.rna, ymax = upper_bound.rna), alpha = 0.4, fill = "red") +
  geom_line(aes(y = fitted.rna_spline), color = "blue") +
  geom_ribbon(aes(ymin = lower_bound.rna_spline, ymax = upper_bound.rna_spline), alpha = 0.4, fill = "blue") +
  theme_bw() +
...
rna_yearFITzoomed <- rna_yearFIT +
  scale_x_continuous(limits = c(0,30),expand = c(0,0))+
  scale_y_continuous(limits = c(0,5), expand = c(0,0))

print(rna_yearFITzoomed)

#### fusion + export ----

trends <- grid.arrange(cd4_yearRAW, cd4_yearFIT, cd4_yearFITzoomed, rna_yearRAW, rna_yearFIT, rna_yearFITzoomed, ncol=3) 

ggsave(plot = trends, filename = "../results/plots_lab/trends_slope.png", width = width_descr*4, height = height_descr*3)

print(trends)

#### final plot ----

cd4_yearFITfinal <- boot_df.cd4 %>%
  ggplot(aes(x = time_diff)) +
  geom_line(data = df.pred, aes(group = factor(id), x = time_diff, y = sqrt(cd4)), color = "grey", alpha = .2) +
  geom_hline(yintercept = sqrt(350)) +
  geom_vline(xintercept = 0, linetype = "dashed")+
  geom_line(aes(y = fitted.cd4_spline), color = "blue") +
  geom_ribbon(aes(ymin = lower_bound.cd4_spline, ymax = upper_bound.cd4_spline), alpha = 0.2, fill = "blue") +
