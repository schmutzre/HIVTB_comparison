...
table1(~ age_at_art_start + gender + cd4_baseline + rna_baseline + who_stage | cohort + incident_tb, data = noTB,
       overall = c(left = "All"),
       topclass="Rtable1-zebra",
       caption = "Baseline characteristics of HIV-positive patients starting antiretroviral treatment (ART) between 2010 and 2022 not presenting with TB at ART start and with no history of TB. Comparing patients with any incident TB case after ART start, with patients without indicent TB after ART start.",
       render.continuous = my.render.cont,
       render.categorical = my.render.cat)
```
...
![Figure 1: Point-wise TB incidence rate estimate stratified by baseline viral load.](images/incidence_rna-05.png)

![Figure 2: Point-wise TB incidence rate estimate stratified by baseline CD4 Count.](images/incidence_cd4-09.png)

![Figure 3: Incident Rate Ratios (IRRs) of Tuberculosis Incidence relative to the Reference Group (CD4 Count ≥ 350 at ART Initiation), compared separately for each cohort.](images/irr-02.png)

## Survival analysis

![Figure 4: The cumulative incidence rates for time to first tuberculosis incidence after ART initiation were calculated using Aalen-Johansen method, adjusting for all-cause death as a competing risk. Stratified by cohort, Khayelitsha (green) and SHCS (orange).](images/incidence_aj-02.png)
...
![Figure 9: Kaplan-Meier curve comparing the survival probability after ART start between South Africa and Switzerland weighted by baseline CD4, Gender and Age at ART start.](images/kaplan_weighted.png)

## CD4 count recovery (IWHOD figure)

![Figure 10: Fitted GAM model using cubic spline estimating the CD4 recovery after ART start.](images/cd4_pred-01.png)

![Figure 11: Fitted GAM model using cubic spline estimating the HIV VL after ART start.](images/rna_pred-01.png)

## Follow-up

![Figure 12: Proportion of patients who died within specified intervals relative to the number of patients present at the start of each interval in RSA. Wilson-type confidence intervals are included.](images/exit_prop-07.png)

![Figure 13: Proportion of cumulative numbers over time.](images/fup_prop-02.png)
