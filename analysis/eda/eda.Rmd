...
complete <- complete %>% 
  mutate(site_tb = case_when(presenting_tb == 0 ~ NA,
                             TRUE ~ site_tb),
...
table1(~ age_at_art_start + gender + site_tb + risk + cd4_baseline + rna_baseline + who_stage + regimen + regimen_tb_group| cohort * presenting_tb, data = complete,
       overall = c(left = "All"),
