...
```{r plot trends cd4 and rna}
#TODO: I want a plot with rawdata (grey) plus simple fitted in red (left), then on the right side one lmer fitted including CI and one spline fitted including CI (right). For both CD4 (top) and RNA (bottom).

df.pred <- ch_lab_long %>%
  filter(time_diff > -30 & time_diff < 100)
...
cd4_yearRAW <- df.pred %>%
  ...
  theme_bw() +
  labs(x = "", title = "raw data + loess") +
  geom_smooth(aes(y = sqrt(cd4)), method = "loess", se = TRUE, color = "red", fill = "blue") +
...
rna_yearRAW <- df.pred %>%
  ...
  geom_smooth(aes(y = log10(rna+1)), method = "loess", se = TRUE, color = "red", fill = "blue") +
  scale_x_continuous(limits = c(-30,100)) +
...
new_data <- data.frame(
  time_diff = seq(min(df.pred$time_diff), max(df.pred$time_diff), length.out = 1000),
  id = rep(unique(df.pred$id)[1], 1000)  # use the first unique id
)

## mixed-effect model

model_cd4 <- lmer(sqrt(cd4) ~ time_diff + (1|id), data = df.pred)

# Bootstrap the predictions
set.seed(123)  # for reproducibility
boot_preds.cd4 <- bootMer(model_cd4, FUN = function(.x) predict(.x, newdata = new_data, re.form = NA), nsim = 100)

# Calculate percentiles for prediction intervals
lower_bound.cd4 <- apply(boot_preds.cd4$t, 2, quantile, probs = 0.025)
upper_bound.cd4 <- apply(boot_preds.cd4$t, 2, quantile, probs = 0.975)
fitted.cd4 <- apply(boot_preds.cd4$t, 2, mean)

# Combine into a data frame
boot_df.cd4 <- data.frame(time_diff = new_data$time_diff, lower_bound.cd4, upper_bound.cd4, fitted.cd4)

## spline model 

# Fit a GAM with a spline term
model_cd4_spline <- gam(sqrt(cd4) ~ s(time_diff) + s(id, bs="re"), data = df.pred, method = "REML")
...
boot_preds.cd4_spline <- boot(data = df.pred, statistic = fit_and_predictCD4, R = 100)

# Calculate percentiles for prediction intervals
lower_bound.cd4_spline <- apply(boot_preds.cd4_spline$t, 2, quantile, probs = 0.025)
upper_bound.cd4_spline <- apply(boot_preds.cd4_spline$t, 2, quantile, probs = 0.975)
fitted.cd4_spline <- apply(boot_preds.cd4_spline$t, 2, mean)

# Combine into a data frame
boot_df.cd4_spline <- data.frame(time_diff = new_data$time_diff, lower_bound.cd4_spline, upper_bound.cd4_spline, fitted.cd4_spline)

# Merge with the existing data frame
boot_df.cd4 <- merge(boot_df.cd4, boot_df.cd4_spline, by = "time_diff")

## Plot 

cd4_yearFIT <- boot_df.cd4 %>%
  ggplot(aes(x = time_diff)) +
  geom_hline(yintercept = sqrt(350)) +
  geom_line(aes(y = fitted.cd4), color = "red") +
  geom_ribbon(aes(ymin = lower_bound.cd4, ymax = upper_bound.cd4), alpha = 0.4) +
  geom_line(aes(y = fitted.cd4_spline), color = "blue") +
  geom_ribbon(aes(ymin = lower_bound.cd4_spline, ymax = upper_bound.cd4_spline), alpha = 0.4) +
  theme_bw() +
  labs(x = "", y = "",title = "LMEM (red) + GAM with a spline (blue)") +
  scale_x_continuous(limits = c(-30,100)) +
  scale_y_continuous(expand = c(0,0), limits = c(0,50)) +
  theme(plot.title = element_text(hjust = 0.5))
...
#### fitted plot HIV-RNA ----
## mixed-effect model
model_rna <- lmer(log10(rna+1) ~ time_diff + (1|id), data = df.pred)

# Bootstrap the predictions
set.seed(123)  # for reproducibility
boot_preds.rna <- bootMer(model_rna, FUN = function(.x) predict(.x, newdata = new_data, re.form = NA), nsim = 100)

# Calculate percentiles for prediction intervals
lower_bound.rna <- apply(boot_preds.rna$t, 2, quantile, probs = 0.025)
upper_bound.rna <- apply(boot_preds.rna$t, 2, quantile, probs = 0.975)
fitted.rna <- apply(boot_preds.rna$t, 2, mean)

# Combine into a data frame
boot_df.rna <- data.frame(time_diff = new_data$time_diff, lower_bound.rna, upper_bound.rna, fitted.rna)

## spline model 

# Fit a GAM with a spline term
model_rna_spline <- gam(log10(rna+1) ~ s(time_diff) + s(id, bs="re"), data = df.pred, method = "REML")
...
# Bootstrap predictions
boot_preds.rna_spline <- boot(data = df.pred, statistic = fit_and_predictRNA, R = 100)

# Calculate percentiles for prediction intervals
lower_bound.rna_spline <- apply(boot_preds.rna_spline$t, 2, quantile, probs = 0.025)
upper_bound.rna_spline <- apply(boot_preds.rna_spline$t, 2, quantile, probs = 0.975)
fitted.rna_spline <- apply(boot_preds.rna_spline$t, 2, mean)

# Combine into a data frame
boot_df.rna_spline <- data.frame(time_diff = new_data$time_diff, lower_bound.rna_spline, upper_bound.rna_spline, fitted.rna_spline)

# Merge with the existing data frame
boot_df.rna <- merge(boot_df.rna, boot_df.rna_spline, by = "time_diff")

## Plot 

rna_yearFIT <- boot_df.rna %>%
  ggplot(aes(x = time_diff)) +
  geom_hline(yintercept = log10(400)) +
  geom_line(aes(y = fitted.rna), color = "red") +
  geom_ribbon(aes(ymin = lower_bound.rna, ymax = upper_bound.rna), alpha = 0.4) +
  geom_line(aes(y = fitted.rna_spline), color = "blue") +
  geom_ribbon(aes(ymin = lower_bound.rna_spline, ymax = upper_bound.rna_spline), alpha = 0.4) +
  theme_bw() +
  labs(x = "Days since ART start", y = "", title = "") +
  scale_x_continuous(limits = c(-30,100)) +
  scale_y_continuous(expand = c(0,0), limits = c(0,10)) +
  theme(plot.title = element_text(hjust = 0.5))
...
trends <- grid.arrange(cd4_yearRAW, cd4_yearFIT, rna_yearRAW, rna_yearFIT, ncol=2) 

ggsave(plot = trends, filename = "../results/plots_lab/trends_slope.png", width = width_descr*2, height = height_descr*2)
...
plot.cd4_tb <- ggplot() +
  geom_point(data = lab_longTBdate_before.cd4, aes(x = timediffTB, y = sqrt(cd4), group = factor(id), color = factor(id)), size = .8, show.legend = FALSE) +
  geom_point(data = lab_longTBdate_after.cd4, aes(x = timediffTB, y = sqrt(cd4), group = factor(id), color = factor(id)), size = .8, show.legend = FALSE) +
  geom_line(data = lab_longTBdate_before.cd4, aes(x = timediffTB, y = sqrt(cd4), group = factor(id), color = factor(id)), alpha = .5, show.legend = FALSE) +
  geom_line(data = lab_longTBdate_after.cd4, aes(x = timediffTB, y = sqrt(cd4), group = factor(id), color = factor(id)), alpha = .5, show.legend = FALSE) +
  geom_line(data = boot_df.cd4before, aes(x = time_diff, y = fitted.cd4before), color = "blue", linewidth = 1) +
  geom_line(data = boot_df.cd4after, aes(x = time_diff, y = fitted.cd4after), color = "red", linewidth = 1) +
  geom_hline(yintercept = sqrt(350)) +
  theme(legend.position = "none") +
...
plot.rna_tb <- ggplot() +
  geom_point(data = lab_longTBdate_before.rna, aes(x = timediffTB, y = log10(rna+1), group = factor(id), color = factor(id)), size = .8, show.legend = FALSE) +
  geom_point(data = lab_longTBdate_after.rna, aes(x = timediffTB, y = log10(rna+1), group = factor(id), color = factor(id)), size = .8, show.legend = FALSE) +
  geom_line(data = lab_longTBdate_before.rna, aes(x = timediffTB, y = log10(rna+1), group = factor(id), color = factor(id)), alpha = .5, show.legend = FALSE) +
  geom_line(data = lab_longTBdate_after.rna, aes(x = timediffTB, y = log10(rna+1), group = factor(id), color = factor(id)), alpha = .5, show.legend = FALSE) +
  geom_line(data = boot_df.rnabefore, aes(x = time_diff, y = fitted.rnabefore), color = "blue", linewidth = 1) +
  geom_line(data = boot_df.rnaafter, aes(x = time_diff, y = fitted.rnaafter), color = "red", linewidth = 1) +
  geom_hline(yintercept = log10(400)) +
  theme(legend.position = "none") +
  scale_x_continuous(limits = c(-360, 360)) +
...
tbtrend.presenting <- grid.arrange(plot.cd4_tb, plot.rna_tb, ncol=2, 
             top = "Lab data with seperately fitted linear-mixed model before and after TB diagnosis (presenting with TB)") 
