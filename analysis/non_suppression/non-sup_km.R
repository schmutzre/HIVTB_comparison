...
kaplan_ch <- readRDS("data_clean/NOsupress_df.rds") %>% 
...
km.surv_obj <- Surv(kaplan$persontime_years.NOsuppression, kaplan$incidence.nosup)
...
ggsave(filename = "results/non_sup/non_sup-km_zoom.png", plot = zoom_plot, width = 10, height = 8, dpi = 300)
...
# Create a survival object with your time and event variables
km.surv_objCH <- Surv(kaplan_ch$persontime_years.NOsuppression, kaplan_ch$incidence.nosup)
...
ggsave(filename = "results/non_sup/non_sup-km_CH.png", plot = zoom_plotCH, width = 10, height = 8, dpi = 300)

#### Aalen Johansen Model ----

# Create the multi-state survival object
aj_fit <- Surv(kaplan$persontime_years.NOsuppression, kaplan$event_type, type = "mstate")
...
axis(2, at = seq(0, 0.06, by = 0.01), 
     labels = formatC(1 - seq(0, 0.06, by = 0.01), digits = 3))
...
png(filename = "results/non_sup/non_sup-AJ-both.png", width = 800, height = 600)
replayPlot(AJ.both)
dev.off()
...
axis(2, at = seq(0, 0.05, by = 0.01), 
     labels = formatC(1 - seq(0, 0.05, by = 0.01), digits = 3))
...
AJ.ch = recordPlot()
png(filename = "results/non_sup/non_sup-AJ-ch.png", width = 750, height = 500)
replayPlot(AJ.ch)
dev.off()
