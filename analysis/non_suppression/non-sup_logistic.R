...
ch <- readRDS("data_clean/NOsupress_df.rds") %>% 
  mutate(agegroup = cut(age_at_ART_start, breaks = custom_breaks, include.lowest = TRUE),
         ...
         dplyr::select(id, incidence.nosup , sex, cohort, agegroup, baselineCD4, baselineRNA)
...
model.log <- glm(incidence.nosup ~ cohort + sex + agegroup + baselineRNA + baselineCD4, 
                 data = logistic, 
                 family = "binomial")
...
plot <- plot_model(model.log, 
  ...
  theme_bw()+
  labs(title = "Risk factors for viral non-suppression") + 
  theme(plot.title = element_text(hjust = 0.5))
plot

ggsave(plot = plot, filename = "results/non_sup/non_sup-odds.png")
...
roc_obj <- roc(response = logistic$incidence.nosup, predictor = predicted_probabilities)  # changed `case_incident_2m` to `incidence` 
...
predicted_class <- logistic %>% 
  mutate(prediction = predicted_probabilities,
         ...
         incidence = as.factor(incidence.nosup))
